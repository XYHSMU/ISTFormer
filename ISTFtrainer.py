...
class trainer:
    def __init__(
        ...
        self.model = ISTFormer(
            device, input_dim, num_nodes, dropout,embeding_dim
        )
        self.model.to(device)
        self.optimizer = Ranger(self.model.parameters(), lr=lrate, weight_decay=wdecay)#这也是一个adam优化器
    ...
    def train(self, input, real_val):
        ...
        if self.tool == 'xyh_util':
            predict = self.scaler.inverse_normal(output)
            pass
        else:
            predict = self.scaler.inverse_transform(output)
            pass

        loss = self.loss(predict, real, 0.0)#util.MAE_torch
        loss.backward()
    ...
    for i in range(1, args.epochs + 1):
        ...
        if mvalid_loss < loss:
            ...
            if i < 100:
                ...
                for j in range(12):
                    if tool == 'xyh_util':
                        pred = scaler.inverse_normal(yhat[:, :, j])
                        pass
                    else:
